...
class paginator
{
	...
	{
		if ( !isset($pages['by_page'] ) ) $pages['by_page'] = 10;
		if ( !isset($pages['varname'] ) ) $pages['varname'] = 'page';
		if ( !isset($pages['url'] ) ) $pages['url'] = self_url();
		
		$pages['count'] = max( 1, intval( $count ) );
		$pages['by_page'] = max( 1, intval( $pages['by_page'] ) );
		
		$query_string = http_build_query( prepare_query( $_GET, array( $pages['varname'] ) ) );
		$page_url = $pages['url'] . '?' . ( $query_string ? $query_string . '&' : '' ) . $pages['varname'] . '=';
		
		$first_page = 0; $last_page = max( floor( ( $pages['count'] - 1 ) / $pages['by_page'] ), 0 );
